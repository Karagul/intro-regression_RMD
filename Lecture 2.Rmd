...
20th century statistical analysis focused on inferring characteristics on the basis of a sample.

Data were expensive to collect, computers were limited by memory and processing speeds.

So we take a sample and analyze that, then generalize to the population.
...
Picture of sample distribution with population parameter marked.


## Null Hypothesis Significance Testing

So a sample mean is almost surely not the population value we really want to know.

The interesting thing is that, while we know we are wrong, we can actually say something aobut how wrong we are likely to be!

This is, for example, the reason why we see survey results published with a _margin of error_.

Our ability to quantify our level of wrongness comes from a result known as the _Central Limit Theorem_.  

This result works if we think of our sample as one of all possible samples of size _N_ that can be taken from a population.
...
```{r slide5, echo = FALSE}
knitr::include_graphics("figures/Slide5.PNG")
```
...
```{r slide6, echo = FALSE}
knitr::include_graphics("figures/Slide6.PNG")
```
...
```{r dist_means, echo = FALSE, out.width = "600px"}
norm_data <- data_frame(x = seq(-3,3,by=.001), y = dnorm(seq(-3,3,by=.001)))
...
ggplot(norm_data, aes(x = x, y = y)) + geom_line() + theme(axis.ticks = element_blank(), axis.text.x = element_blank(), axis.text.y = element_blank()) + 
  labs(x = "Sample Means", y = "Frequency of Occurrence")
...
> As the sample size _n_ gets larger, the sampling distribution of the mean (and many other statistics) converges to a normal distribution.


## The Central Limit Theorem

This result does not depend on the distribution of scores in a population.  Take, for example, the distribution of students across grades in a high school.

```{r students, echo = FALSE, out.width = "650px"}
...
But there is more.  The CLT tells us the shape of the distribution, but it is also relatively easy to show that
...
That is, the amount of sample-to-sample variability depends on the standard deviation of population scores and the size of the sample we draw.
...
If I know the population distribution, I know the shape of the sampling distribution almost _exactly_ (provided $n$ is sufficiently large).

With this information, I can say how likely it is that a single sample was generated from a given population distribution.
...
If Adderall has an effect, then Adderall sniffers come from a different population with its own different mean.  What should we infer?
...
Our sample has a mean IQ of 103.  How likely is it that we would observed this value if our sample were no different from the population?

```{r iq_plot, echo = FALSE}
xbar <- 103
xmin <- 93
xmax <- 107
mu   <- 100
...
What if we're amibivalent?

```{r alpha05both, echo = FALSE}
...
In other words, the more data we have, the greater certainty we can have in an estimate.  The next sample we draw will not be as wildly different from the current sample given a larger $n$.
...
Notice that in each of these graphs we denote the area under the curve in the tails with $p$, for probability of observing a statistic at least this far from the population mean.
...
ggplot(norm_data, aes(x = x, y = y)) + geom_line() +
  ...
  labs(x = "Sample Means Under Null Hypothesis", y = "Density", title = "N = 100") +
  geom_segment(aes(x = xbar, y = 0, xend = xbar, yend = .04), size = 1.25) + 
  geom_text(x = xbar, y = .05, label = "z  = 1.667")
...
- Historically, needed to rely on tables to find there area under the sampling distribution curve. 
...
Before adding in the last bit you need to know for hypothesis testing, make sure you understand the following:
...
2. We can therefore assert a null hypothesis and know what the sampling distribution under the null looks like.


## Review

3. If our sample mean $M$ is sufficiently unlikely under the null hypothesis, we reject the null.
4. _Sufficiently unlikey_ is below a probability threshold $\alpha$, usually set at .05 and distributed between the upper and lower tails of the distribution.
5. We can convert our mean to a $z$ statistic if that makes it easier to find the associated $p$-value.

This rests on knowing $\mu$ and $\sigma$.  What if we don't know $\sigma$?
...
```{r dv, echo = FALSE, message=FALSE, warning=FALSE}
...
These data are not normally distributed.  Should we care?
...
The $t$ distribution, like the unit normal, is centered on zero.  We therefore convert our estimate of the mean to be a $t$ statistic.  This works just like $z$ statistic:
...
```{r txmple, echo = FALSE, message=FALSE, warning=FALSE}
...
But sample size also matters because it affects the degrees of freedom, which in turn affects the specific $t$ distribution we utilize.
...
```{r n25, echo = FALSE, message=FALSE, warning=FALSE}
...
```{r n50, echo = FALSE, message=FALSE, warning=FALSE}
...
```{r n100, echo = FALSE, message=FALSE, warning=FALSE}
...
```{r n1000, echo = FALSE, message=FALSE, warning=FALSE}
